...
module.exports = {
  ...
  bower: function (done, bwrData) {
    ...
    gulp.src('./branch/package/.travis.yml')
      .pipe(gulp.dest(DIR + '/package/'))
      .on('end', almostDone);

    gulp.src('./branch/package/package.tmpl.json')
  ...
  subbower: function (moduleName, bwrData) {
    return function (done) {
      ...
      var almostDone = cm._.after(5, done);
      var destination = DIR + '/subpackage/' + moduleName;
      ...
      gulp.src('./branch/package/package.tmpl.json')
        .pipe(cm.processTemplateFile({ bwr : cm._.assign({}, cm.pkg, bwrData) }))
        .pipe(rename('package.json'))
        .pipe(gulp.dest(destination))
        .on('end', almostDone);
